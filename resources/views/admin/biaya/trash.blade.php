...
@section('title', 'Tong Sampah - Biaya')

@section('breadcrumb')
    <h4 class="mb-0">Biaya Pendidikan (Trash)</h4>
...
<div class="card card-modern border-0 shadow-sm">
    <div class="card-header bg-transparent border-0 pt-4 pb-3 d-flex justify-content-between align-items-center">
        ...
        <div>
            <a href="{{ route('admin.biaya.index') }}" class="btn btn-outline-secondary">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td>
                                <div class="fw-bold">{{ $item->jenis_biaya }}</div>
                            </td>
                            <td>{{ $item->deleted_at->format('d M Y, H:i') }}</td>
                            <td class="text-end">
                                <form action="{{ route('admin.biaya.restore', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Kembalikan data ini ke daftar utama?')">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-sm btn-success" title="Restore"><i class="fas fa-undo me-1"></i> Restore</button>
                                </form>
                                <form action="{{ route('admin.biaya.force-delete', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Data yang dihapus permanen tidak dapat dikembalikan. Lanjutkan?')">
                                    @csrf
                                    @method('DELETE')
