...
@section('title', 'Tong Sampah - Fasilitas')

@section('breadcrumb')
    <h4 class="mb-0">Fasilitas (Trash)</h4>
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td class="text-end">
                                <form action="{{ route('admin.fasilitas.restore', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Kembalikan data ini ke daftar utama?')">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-sm btn-success" title="Restore"><i class="fas fa-undo me-1"></i> Restore</button>
                                </form>
                                <form action="{{ route('admin.fasilitas.force-delete', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Data yang dihapus permanen tidak dapat dikembalikan. Lanjutkan?')">
                                    @csrf
                                    @method('DELETE')
