...
@section('title', 'Tong Sampah - Jadwal PMB')

@section('breadcrumb')
    <h4 class="mb-0">Jadwal PMB (Trash)</h4>
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td>
                                <div class="fw-bold">{{ $item->kegiatan }}</div>
                            </td>
                            <td>{{ $item->gelombang }}</td>
                            <td>{{ $item->deleted_at->format('d M Y, H:i') }}</td>
                            <td class="text-end">
                                <form action="{{ route('admin.jadwal-pmb.restore', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Kembalikan data ini ke daftar utama?')">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-sm btn-success" title="Restore"><i class="fas fa-undo me-1"></i> Restore</button>
                                </form>
                                <form action="{{ route('admin.jadwal-pmb.force-delete', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Data yang dihapus permanen tidak dapat dikembalikan. Lanjutkan?')">
                                    @csrf
                                    @method('DELETE')
