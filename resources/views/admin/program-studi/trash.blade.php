...
@section('title', 'Tong Sampah - Program Studi')

@section('breadcrumb')
    <h4 class="mb-0">Program Studi (Trash)</h4>
...
<div class="card card-modern border-0 shadow-sm">
    <div class="card-header bg-transparent border-0 pt-4 pb-3 d-flex justify-content-between align-items-center">
        ...
        <div>
            <a href="{{ route('admin.program-studi.index') }}" class="btn btn-outline-secondary">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr>
                            ...
                            <td class="text-end">
                                <form action="{{ route('admin.program-studi.restore', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Kembalikan data ini ke daftar utama?')">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-sm btn-success" title="Restore"><i class="fas fa-undo me-1"></i> Restore</button>
                                </form>
                                <form action="{{ route('admin.program-studi.force-delete', $item->id) }}" method="POST" class="d-inline" onsubmit="return confirm('PERINGATAN! Data yang dihapus permanen tidak dapat dikembalikan. Lanjutkan?')">
                                    @csrf
                                    @method('DELETE')
